        ...
        const swpier = (delay: number) => {
            const timer = setTimeout(() => {
                ...
                if (index > len - 1) {
                    ...
                    index = 0;
                    top = 0;
                    this.__renderer2.setStyle(this.__contentBoxRef.nativeElement, 'transition', 'none');
                    swpier(1000);
                } else {
                    top = -1 * itemHeight * index;
                    this.__renderer2.setStyle(this.__contentBoxRef.nativeElement, 'transition', 'top 0.5s');
                    // 如果是最后一个复制块，间隔时间需要修改，加上最上面的替换块，合计2s。
                    index === len - 1 ? swpier(1000) : swpier(2000);
                }

                this.__renderer2.setStyle(this.__contentBoxRef.nativeElement, 'top', `${top}px`);
            }, delay);
        };
